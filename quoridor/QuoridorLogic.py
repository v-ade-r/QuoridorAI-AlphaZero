...
def is_wall_legal(moves):
    ...
    if moves == [0, 1]:
        return 1
    if isinstance(moves, (bytes, bytearray)):
        return 0
...
class Board:
    def __init__(self, n):
        ...
        self.pieces = np.zeros((4, self.n, self.n), dtype=int)
    ...
    def get_legal_moves(self, color, pool=None):
        ...
        wall_moves = self.wall_moves(color, pool)
        moves += wall_moves
    ...
    def wall_moves_(self, color):
        """Returns legal wall moves."""
        idx = 2 if color == 1 else 3
        if np.sum(self.pieces[idx]) == 20:  # All available walls used by a player
            return [0] * (self.n_ ** 2) * 2

        return self.wall_moves(color, pool=None)
    ...
    def wall_moves(self, color, pool):
            ...
            for y in range(0, self.n-2, 2):
                ...
                if (not block[x][y] and not block[x][y+2] and
                        ((not block[x-1][y+1] or not block[x+1][y+1]) or
                         (x-3 >= 0 and x+3 < self.n and block[x-1][y+1] and
                          block[x-3][y+1] and block[x+1][y+1] and block[x+3][y+1]))):
        ...
        for x in range(2, self.n, 2):
            for y in range(1, self.n, 2):
                ...
                if (not block[x][y] and not block[x-2][y] and
                        ((not block[x-1][y-1] or not block[x-1][y+1]) or
                         (y-3 >= 3 and y+3 < self.n and block[x-1][y-1] and
                          block[x-1][y-3] and block[x-1][y+1] and block[x-1][y+3]))):
        ...
        if np.sum(moves_boolean) > len(moves_boolean)/3:
            ...
            if parent != 0 and parent_ != 0:
                valid_walls = (self.walls_not_in_path(parent, tail) & self.walls_not_in_path(parent_, tail_) &
                               moves_boolean)
        ...
        if pool is None:
            return [self.is_wall_legal(move[1:]) if move is not None else 0 for move in moves]
